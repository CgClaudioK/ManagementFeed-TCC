                        ...
                        <tbody>
                            @forelse ( $insumos as $insumo )
                                <tr>
                                    ...
                                    <td class="font-normal px-4 py-2 text-center">{{ $insumo->produto->nome_produto ?? 'Não encontrado'  }}</td>
                                    ...
                                    <div class="flex justify-center gap-2">
                                        <a href="{{route('admin.insumos.edit', ['insumo'=> $insumo->id])}}" class="px-2 py-1 text-black dark:text-white">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <form action="{{ route('admin.insumos.destroy', ['insumo' => $insumo->id]) }}" method="POST" id="delete-form-{{ $insumo->id }}">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" onclick="confirmDeletion(event, 'delete-form-{{ $insumo->id }}')" class="px-2 py-1 text-black dark:text-white">
